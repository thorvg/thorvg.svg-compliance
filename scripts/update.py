...
def prepare_corpora(suites, sources, thorvg, accept_baseline):
    ...
    suite = suites["thorvg-regression"]
    target = destination / suite["id"]
    old = ROOT / "corpora" / suite["id"]
    if not accept_baseline:
        copy_tree(old / "reference", target / "reference")
        if (old / "baseline.json").exists():
            copy_file(old / "baseline.json", target / "baseline.json")
    start = len(records)
    for svg in sorted((thorvg["path"] / "test/resources").glob("*.svg")):
        relative = Path(svg.name)
        copy_file(svg, target / "svg" / relative)
        text = read_svg(svg)
        record = base_record(
            suite, relative, f"corpora/{suite['id']}/svg/{relative.as_posix()}",
            f"https://github.com/thorvg/thorvg/blob/{thorvg['commit']}/test/resources/{relative.as_posix()}",
            title(text, relative.stem), elements(text), tuple(suite["resolution"]),
        )
        record.update({
            "feature": "regression", "reference_image": f"corpora/{suite['id']}/reference/{relative.with_suffix('.png').as_posix()}",
            "_source_path": target / "svg" / relative,
            "_reference_path": target / "reference" / relative.with_suffix(".png"),
            "_baseline_pending": accept_baseline or not (target / "reference" / relative.with_suffix(".png")).exists(),
        })
        records.append(record)
    copy_file(thorvg["path"] / "LICENSE", target / "LICENSE")
    count = len(records) - start
    (target / "README.md").write_text(suite_readme(suite, thorvg["commit"], count))
    metadata.append({**suite, "revision": thorvg["commit"], "index": f"corpora/{suite['id']}/README.md"})
...
def finalize(records, suites, corpora, tool, accept_baseline, thorvg_commit):
    ...
    baseline_suite = corpora / "thorvg-regression"
    baseline_changed = False
    for record in records:
        if record.get("_baseline_pending") and record["_rendered_path"].exists():
            copy_file(record["_rendered_path"], record["_reference_path"])
            baseline_changed = True
    if baseline_changed or accept_baseline:
        info = {"accepted_at": datetime.now(timezone.utc).isoformat(), "thorvg_commit": thorvg_commit, "note": "Generated by scripts/update.py"}
        (baseline_suite / "baseline.json").write_text(json.dumps(info, indent=2) + "\n")
...
def parse_args():
    ...
    parser.add_argument("--accept-regression-baseline", action="store_true", help="Replace the ThorVG regression goldens")
...
def main():
    ...
    corpora, records, metadata = prepare_corpora(suites, sources, thorvg, args.accept_regression_baseline)
    assets, records = finalize(records, suites, corpora, tool, args.accept_regression_baseline, thorvg["commit"])
